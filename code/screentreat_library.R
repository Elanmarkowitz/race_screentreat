...
############################################################
## format_clinical_incidence_for_race
############################################################

# Assumes the cantrance package is loaded

format_clinical_incidence_for_race <- function(
  incfile,
  ### Path to SEER clinical incidence table
  race
  ### vector of races
  ...
  Crude_Rate_col <- paste0('Crude.Rate.', race)
  age_max <- max(seer$Age)
  incidence <- 
    data.frame(age = 0:age_max, 
               incidence_rate = approx(seer$Age,
                                       as.numeric(seer[Crude_Rate_col][ , 1]),
                                       xout=0:age_max)$y/100000)
...
############################################################
## return_value_from_id_by_race
############################################################

return_value_from_id_by_race = function# Use an ID no to look up a value dependent on race

##description<< Using a matrix of IDs, look up a value dependent on race in a dataframe

(matrixname,
 ### id of matrix in ids list
 ids,
 ### List of matrices containg the IDs
 df,
 ### Data frame containing the values of interest, sorted
 ### by ID (i.e. row #s == ID #s)
 value
 ### Vector containing column name of value to return from the df for each column in ids
) {
  
  return(apply(ids[[matrixname]], 2, function(x, val)df[,val][x], value[matrixname]))
