...
# Identify races
races <- sub('HR_','',
              grep('HR', colnames(treat_chars), value=TRUE))

trial_race <- c()
for (i in trials){
  trial_race[i] <- sub("([^_]+)_.+", "\\1", i) 
}
...
for (i in races){
    control_notreat[[i]] <- transform(control_notreat[[i]],
                                 SSno=1:nrow(control_notreat[[i]]),
                                 SSid=paste(stage, subgroup, 
                                               sep='.'))
}

treat_chars_temp <- transform(treat_chars,
                              SSid=paste(stage, subgroup, sep='.'))
                         
treat_chars <- list()
for (i in races){
    treat_chars[[i]] <- merge(treat_chars_temp,
                              subset(control_notreat[['White']], 
                                     select=-c(stage, subgroup)),
                              by='SSid',
                              all.x=TRUE, sort=FALSE)
    
    # Now for treatment-stage-subgroups
    treat_chars[[i]] <- transform(treat_chars[[i]],
                                  txSSid=paste(SSid,tx,sep='.'),
                                  txSSno=1:nrow(treat_chars[[i]]))
}
rm(treat_chars_temp)
...
control_notreat_rows <- list()
for (i in races){
    control_notreat_rows[[i]] <- sim_multinom(nsims=pop_size, 
                                              nreps=nsim, 
                                              probs=control_notreat[[i]]$prop, 
                                              names=1:nrow(control_notreat[[i]]))
}
